/*
 * Copyright (C) 2004 Joe Walnes. Copyright (C) 2006, 2007 XStream Committers. All rights reserved.
 * 
...
package org.openqa.selendroid.android.internal;
...
public class Base64Encoder {
  ...
  public String encode(byte[] input) {
    ...
    for (int i = 0; i < input.length; i += 3) {
      ...
      for (int j = 0; j < 4; j++)
        result.append(remaining + 1 > j
            ? SIXTY_FOUR_CHARS[0x3f & oneBigNumber >> 6 * (3 - j)]
            : '=');
  ...
  public byte[] decode(String input) {
    try {
      ...
      for (int i = 0; i < input.length(); i += 4) {
        ...
        for (int j = 0; j < 3; j++)
          if (a[j + 1] >= 0) out.write(0xff & oneBigNumber >> 8 * (2 - j));
      }
