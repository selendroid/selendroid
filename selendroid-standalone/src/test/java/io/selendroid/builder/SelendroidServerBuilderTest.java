...
public class SelendroidServerBuilderTest {
	...
	private static final String SELENDROID_PREBUILD_SERVER = "src/test/resources/selendroid-server.apk";
	public static final String ANDROID_APPLICATION_XML_TEMPLATE = "src/main/resources/AndroidManifest.xml";

	@Test
	public void testShouldBeAbleToCreateCustomizedSelendroidServerAndCleantTUp()
			throws Exception {
		SelendroidServerBuilder builder = new SelendroidServerBuilder(
				SELENDROID_PREBUILD_SERVER, ANDROID_APPLICATION_XML_TEMPLATE);
		builder.init(APK_FILE);
		builder.cleanUpPrebuildServer();

		// Verify apk, if the files have been removed
		String line = AndroidSdk.aapt() + " list "
	...
	@Test
	public void testShouldBeAbleToCreateCustomizedAndroidApplicationXML()
			throws Exception {
		SelendroidServerBuilder builder = new SelendroidServerBuilder(
				SELENDROID_PREBUILD_SERVER, ANDROID_APPLICATION_XML_TEMPLATE);
		builder.init(APK_FILE);
		...
		Assert.assertTrue("Expecting non empty AndroidManifest.xml file",
				entry.getSize() > 700);
	...
	@Test
	public void testShouldBeAbleToCreateASignedSelendroidServer()
			throws Exception {
		SelendroidServerBuilder builder = new SelendroidServerBuilder(
				SELENDROID_PREBUILD_SERVER, ANDROID_APPLICATION_XML_TEMPLATE);
		builder.init(APK_FILE);
		builder.cleanUpPrebuildServer();
		File file = builder.signTestServer(builder
				.createAndAddCustomizedAndroidManifestToSelendroidServer());
